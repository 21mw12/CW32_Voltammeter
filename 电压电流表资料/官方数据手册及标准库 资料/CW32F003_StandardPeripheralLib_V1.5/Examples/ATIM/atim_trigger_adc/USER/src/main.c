...
int32_t main(void)
{
  ...
  RCC_Configuration(); // HSI时钟源，6分频，PCLK=HCLK=8MHz
  ...
  while (1)
  {
    ...
    {
      ADC_ConvertedFlag = 0;
      ADC_ConvertedVoltage = ADC_ConvertedValue / 0xfff * 3.3;
    }
  }
}
...
static void ADC_Config(void)
{
  ...
  ADC_InitStruct.ADC_ClkDiv = ADC_Clk_Div1; // ADCCLK=8MHz
  ...
  ADC_InitStruct.ADC_VrefSel = ADC_Vref_VDD;
