...
/**
 * @brief  使用GTIM的CH1和CH2两个通道对PWM输入信号进行测量，CH1测量周期，
 *         CH2测量脉宽。
 *         GTIM_CH1：PB00
 *         GTIM_CH2: PB01
 *         将同一个PWM输入信号引入到PB00和PB01上。
 * @return int32_t 
 */
int32_t main(void)
{
...
void GTIM_IRQHandlerCallBack(void)
{
  ...
  if (GTIM_GetITStatus(GTIM_IT_CC1))
  {
    if (stage == 0)
    {
      PWMPeriod = GTIM_GetCapture1();
      stage = 1;
    }
    else if (stage == 1)
    {
      PWMPeriod = GTIM_GetCapture1() + cnt * 65536 - PWMPeriod;
      stage = 0;
      cnt = 0;
    }
    GTIM_ClearITPendingBit(GTIM_IT_CC1);
  }

  if (GTIM_GetITStatus( GTIM_IT_CC2))
  {
    if (stage == 1)
    {
          PWMWidth = GTIM_GetCapture2() + cnt * 65536 - PWMPeriod;
          (void) PWMWidth;
    }
    GTIM_ClearITPendingBit(GTIM_IT_CC2);
  }
}
