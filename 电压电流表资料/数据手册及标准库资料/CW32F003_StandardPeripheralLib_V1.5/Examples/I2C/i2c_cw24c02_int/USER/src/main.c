 ...
 * 
 */ 
void I2cEepromReadWriteInterruptFunction(void)
{
    ...
    switch(u8State)
    {
        case 0x08:     //发送完START信号
            ...
            I2C_Send7bitAddress(I2C_SLAVEADDRESS,0X00);
            break;
        case 0x10:     //发送完重复起始信号
            I2C_GenerateSTART(DISABLE);
            if(0 == SendFlg)
            {
                I2C_Send7bitAddress(I2C_SLAVEADDRESS,0X00);    //写命令
            }
            else
            {
                I2C_Send7bitAddress(I2C_SLAVEADDRESS,0X01);    //读命令,eeprom 随机读
            }
            break;
        case 0x18:    //发送完SLA+W/R字节
            I2C_GenerateSTART(DISABLE);
            I2C_SendData(u8Addr);   //发送访问EEPROM的目标地址字节
            break;
        ...
        case 0x58:    //接收到一个数据字节，且NACK已回复
						u8Recdata[u8RecvLen++] = I2C_ReceiveData();//所有数据读取完成，NACK已发送
						receivedflag =1;
            I2C_GenerateSTOP(ENABLE);//发送停止条件
            break;				
        case 0x28:     //发送完1字节数据：发送EEPROM中memory地址也会产生，发送后面的数据也会产生											
						if(0 == SendFlg)
            {
								...
								{
									I2C_SendData(u8Senddata[u8SendLen++]);
								}
								else
								{
									u8SendLen = 0;
									Comm_flg = 1;
									SendFlg = 1;
									I2C_GenerateSTOP(ENABLE);//发送完数据，发送停止信号
								}
            }
        ...
        case 0x40:     //发送完SLA+R信号，开始接收数据
            u8RecvLen = 0;
						if(READLEN>1)
						{
							I2C_AcknowledgeConfig(ENABLE);//读取数据超过1个字节才发送ACK
						}
            break;	
        case 0x50:     //接收完一字节数据，在接收最后1字节数据之前设置AA=0;
						u8Recdata[u8RecvLen++] = I2C_ReceiveData();
